...
- Blank required params (`meet`, `club`, `wso`, `name`, `query`, `year`) fail with `400` only for strict clients via `ClientVersion`; legacy clients keep the pre-2026-09-08 answers. Name lists stay always-strict. The `/meets/package` cache is invalidated by a per-meet freshness stamp (row counts + max `xmin`), with the TTL as a backstop.
